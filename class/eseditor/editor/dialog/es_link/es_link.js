...
// Set the dialog tabs.
window.parent.AddTab( 'Info', ESLang.DlgLnkInfoTab ) ;

if ( !ESConfig.LinkDlgHideTarget )
	window.parent.AddTab( 'Target', ESLang.DlgLnkTargetTab, true ) ;

if ( ESConfig.LinkUpload )
	window.parent.AddTab( 'Upload', ESLang.DlgLnkUpload, true ) ;

if ( !ESConfig.LinkDlgHideAdvanced )
...
window.onload = function()
{
	...
	GetE('divBrowseServer').style.display = ESConfig.LinkBrowser ? '' : 'none' ;
	...
	if ( ESConfig.LinkUpload )
		GetE('frmUpload').action = ESConfig.LinkUploadURL ;
...
function LoadAnchorNamesAndIds()
{
	...
	var oImages = oEditor.ES.EditorDocument.getElementsByTagName( 'IMG' ) ;
	for( var i = 0 ; i < oImages.length ; i++ )
	{
		if ( oImages[i].getAttribute('_esanchor') )
			aAnchors[ aAnchors.length ] = oEditor.ES.GetRealElement( oImages[i] ) ;
	}
	
	var aIds = oEditor.ESTools.GetAllChildrenIds( oEditor.ES.EditorDocument.body ) ;
	...
	for ( var i = 0 ; i < aAnchors.length ; i++ )
	{
		...
		if ( sName && sName.length > 0 )
			oEditor.ESTools.AddSelectOption( document, GetE('cmbAnchorName'), sName, sName ) ;
	}

	for ( var i = 0 ; i < aIds.length ; i++ )
	{
		oEditor.ESTools.AddSelectOption( document, GetE('cmbAnchorId'), aIds[i], aIds[i] ) ;
	}
...
function LoadSelection()
{
	...
	var sHRef = oLink.getAttribute( '_essavedurl' ) ;
	if ( !sHRef || sHRef.length == 0 )
		sHRef = oLink.getAttribute( 'href' , 2 ) + '' ;
	
	// TODO: Wait stable version and remove the following commented lines.
//	if ( sHRef.startsWith( ES.BaseUrl ) )
//		sHRef = sHRef.remove( 0, ES.BaseUrl.length ) ;
...
function Ok()
{
	...
	switch ( GetE('cmbLinkType').value )
	{
		...
		case 'anchor' :
			...
			if ( sAnchor.length == 0 )
			{
				alert( ESLang.DlnLnkMsgNoAnchor ) ;
				return false ;
			}
	...
	{
		oLink = oEditor.ES.CreateLink( sUri ) ;
		if ( ! oLink )
			return true ;
	}
	
	SetAttribute( oLink, '_essavedurl', sUri ) ;
...
{
	OpenFileBrowser( ESConfig.LinkBrowserURL, ESConfig.LinkBrowserWindowWidth, ESConfig.LinkBrowserWindowHeight ) ;
}
...
var oUploadAllowedExtRegex	= new RegExp( ESConfig.LinkUploadAllowedExtensions, 'i' ) ;
var oUploadDeniedExtRegex	= new RegExp( ESConfig.LinkUploadDeniedExtensions, 'i' ) ;
...
function CheckUpload()
{
	...
	if ( ( ESConfig.LinkUploadAllowedExtensions.length > 0 && !oUploadAllowedExtRegex.test( sFile ) ) ||
		( ESConfig.LinkUploadDeniedExtensions.length > 0 && oUploadDeniedExtRegex.test( sFile ) ) )
	{
