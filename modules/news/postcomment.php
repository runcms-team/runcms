...
case "post":
	if ( !empty($_POST['comment_id']) ) {
		$artcomment = new RcxComments($db->prefix("comments"), $_POST['comment_id']);
		$accesserror = 0;
	if ( $rcxUser ) {
	...
	if ( $accesserror == 1 ) {
		redirect_header("article.php?storyid=".intval($_POST['item_id'])."&comment_id=".intval($_POST['comment_id'])."&order=".intval($_POST['order']),1,_NW_EDITNOTALLOWED);
		exit();
	}
			...
			$uid = $rcxUser->getVar("uid");
				...
				if ( $rcxConfig['anonpost'] == 1 ) {
					$uid = 0;
						...
						redirect_header("article.php?storyid=".$_POST['itemid']."&comment_id=".$_POST['comment_id']."&order=".$_POST['order'],1,_NW_ANONNOTALLOWED);
						exit();
					}
		}
	...
	$artcomment->setVar("subject", $_POST['subject']);
	$artcomment->setVar("comment", $_POST['message']);
	...
	$artcomment->setVar("icon", $_POST['icon']);
	$newtid = $artcomment->store();

	redirect_header("article.php?storyid=".intval($_POST['item_id'])."&comment_id=".$newtid."&order=".intval($_POST['order'])."#".$newtid."",2,_NW_THANKSFORPOST);
	exit();
	break;
}
