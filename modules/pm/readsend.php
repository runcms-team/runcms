...
$pm_arr         =& PM::getAllPM(array("from_userid=".$rcxUser->getVar("uid").""), true, $sort, $by, 1, $start);
...
if ( empty($pm_arr) ) {
	echo "<tr><td class='bg3' colspan='2' align='center'>"._PM_YOUDONTHAVE."</td></tr>";
		...
		$poster = new RcxUser($pm_arr[0]->getVar("to_userid"));
		if ( !$poster->isActive() ) {
			$poster = false;
		}
		...
		if ($file != '')
{
	...
	echo "<br><br>";
		...
		if ( $previous >= 0 ) {
			echo "<a href='readsend.php?start=".$previous."&amp;total_pm=".$total_pm."&amp;sort=".$sort."&amp;by=".$by."'>"._PM_PREVIOUS."</a> | ";
			} else {
				echo _PM_PREVIOUS." | ";
			}
		if ($next < $total_pm) {
			echo "<a href='readsend.php?start=".$next."&amp;total_pm=".$total_pm."&amp;sort=".$sort."&amp;by=".$by."'>"._PM_NEXT."</a>";
			} else {
				echo _PM_NEXT;
			}
		echo "
		...
		echo "&nbsp;&nbsp;<img style='cursor: hand;' onClick='javascript:openWithSelfMain(\"print.php?msg_id=".$pm_arr[0]->getVar("msg_id")."&op=print_pn\",\"Drucken\",700,500)' src='".RCX_URL."/modules/pm/images/print.gif' alt='"._PM_PRINT."' >   
		</td></tr>";
	}
echo '</table>';
