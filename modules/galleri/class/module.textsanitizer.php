...
if(!defined("GALL_TEXTSANITIZER_INCLUDED")){
  define("GALL_TEXTSANITIZER_INCLUDED",1);

class GallTextSanitizer {
  ...
  */
    ...
    if ( count($this->smileys) == 0 ) {
      if ($getsmiles = $db->query("SELECT * FROM ".$db->prefix("smiles"))){
        while ($smiles = $db->fetch_array($getsmiles)) {
          $message = str_replace($smiles['code'], "<img src='".RCX_URL."/images/smilies/".$smiles['smile_url']."' />", $message);
          array_push($this->smileys, $smiles);
        }
      }
    } else {
      foreach ( $this->smileys as $smile ) {
        $message = str_replace($smile['code'], "<img src='".RCX_URL."/images/smilies/".$smile['smile_url']."' />", $message);
      }
    }
    return $message;
  }
  ...
  function desmile($message) {
    ...
    if ( count($this->smileys) == 0 ) {
      if ($getsmiles = $db->query("SELECT * FROM ".$db->prefix("smiles"))){
        while ($smiles = $db->fetch_array($getsmiles)) {
          $message = str_replace("<img src='".RCX_URL."/images/smilies/".$smiles['smile_url']."' />", $smiles['code'], $message);
          array_push($this->smileys, $smiles);
        }
      }
    } else {
      foreach ( $this->smileys as $smile ) {
        $message = str_replace("<img src='".RCX_URL."/images/smilies/".$smiles['smile_url']."' />", $smile['code'], $message);
      }
    }
    return $message;
  }
