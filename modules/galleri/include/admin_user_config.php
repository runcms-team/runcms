...
*/
    if ( defined('GALL_ADMIN') ) {
        ...
        case "addUser":
          $size = sizeof($uids);
          for ( $i = 0; $i < $size; $i++ ) {
            $sql = "INSERT INTO ".$db->prefix("galli_user")." (cid, uid) VALUES (".$cid.",".$uids[$i].")";
            $db->query($sql); 
          }
          redirect_header("index.php?op=User_config&cid=".$cid."",1,_AD_MA_DBUPDATED);
          break;
        
        case "delUser":
          $size = sizeof($uids);
          for ( $i = 0; $i < $size; $i++ ) {
            $sql = "DELETE FROM ".$db->prefix("galli_user")." WHERE uid=".$uids[$i]." AND cid=".$cid."";
            $db->query($sql); 
          }
          redirect_header("index.php?op=User_config&cid=".$cid."",1,_AD_MA_DBUPDATED);
          break;    
          ...
          default:
            gall_cp_header();
              ...
              titel_Verz(_AD_MA_RECHTETEXT2, _AD_MA_CATMENUE, "User_config");
                if ($cid > 0){
                  ...
                  echo "<td width=3%>&nbsp;</td>";  
                  echo "<td><font size='2'><b>"._AD_MA_USERTEXT2.$cname."</b><font>";
                  echo "</td><td width=3%>&nbsp;</td></tr></table>";
                  $sql = "SELECT l.uid, u.uname FROM ".$db->prefix("galli_user")." l, ".$db->prefix("users")." u WHERE l.cid = ".$cid." AND l.uid=u.uid ORDER BY uname";
                  $result = $db->query($sql);
                  $admins= array();
                  ...
                  <td align='center'>
                  <input type='hidden' name='op' value='User_config' />
                    <input type='hidden' name='op_pref' value='addUser' />
                  <input type='hidden' name='cid' value='".$cid."' />
                  <input type='submit' name='submit' value='"._AD_ADDBUTTON."' />
                  </form><br />
                  <form action='index.php' method='post' />
                  <input type='hidden' name='op' value='User_config' />
                    <input type='hidden' name='op_pref' value='delUser' />
                  <input type='hidden' name='cid' value='".$cid."' />
                  ...
                  echo "</td></tr><tr><td class='bg2'>

				  &nbsp;</td></tr></table>";
                }else{
                  echo "<table width=100% border=0 cellspacing=0 cellpadding=2 class='bg1'><tr>";
                  echo "<td width=3%>&nbsp;</td>";  
                  echo "<td><font size='2'><b>"._AD_MA_USERTEXT3."</b><font>";

                  echo "</td><td width=3%>&nbsp;</td></tr></table>";
                  $sql = "SELECT l.uid, u.uname FROM ".$db->prefix("galli_user")." l, ".$db->prefix("users")." u WHERE l.cid = '0' AND l.uid=u.uid ORDER BY uname";
                  $result = $db->query($sql);
                  $admins= array();
                  while($myrow = $db->fetch_array($result)){
                    array_push($admins, $myrow);
