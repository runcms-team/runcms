    ...
    if ( defined('GALL_ADMIN') || defined('GALL_CO_ADMIN') ) {
        ...
        switch($op_pref){
          ...
          case "save":
            ...
            if ($coment == ""){$eh->show("0105");}
                ...
                if (!$result){
                    ...
                    redirect_header("".GALL_PAGE."?op=cat_conf&op_coad=".$op_coad, 1, _AD_MA_DBUPDATED);
                }
          break;
          ...
          default:
            ...
            include_once(GALLI_PATH."/class/gall_cat.php");
            titel_Verz(_AD_MA_TITEL, _AD_MA_TITEL2, "cat_conf", 1);
            $galltree = new GalTree($db->prefix("galli_category"),"cid","scid");
            $result = mysql_query("SELECT * FROM ".$db->prefix("galli_category")." ORDER BY cname");
            $result_cat = $result;    
            ...
            if (!mysql_num_rows($result_cat)){
              echo " <tr bgcolor='".$bgcolor1."'><td colspan=7 align=center>"._AD_MA_NOCAT."</td></tr>\n";
                ...
                $mytree = new GalTree($db->prefix("galli_category"),"cid","scid");
            $co_admin_catid = $mytree->makeCatCoAdmin("cname", "cname", $op_coad, 0, "", "", $catorimg);
            foreach ($co_admin_catid as $cid) {
                    $result_cat = $db->query("SELECT * FROM ".$db->prefix("galli_category")." WHERE cid=$cid");
                  $val = $db->fetch_array($result_cat);
//                while ($val = mysql_fetch_array($result_cat)) {
                    $cid_temp = $val[cid];
                    echo "<tr class='bg1'><td><a href='".GALL_PAGE."?op=modCat&amp;cid=".$val[cid]."&amp;op_coad=".$op_coad."'>".$val[cname]."</a></td>\n";
                  if ($val[img]==''){
        ...
        $galltree = new GallTree($db->prefix("galli_category"),"cid","scid");
            ...
            OpenTable();
                $form = new RcxThemeForm(_AD_MA_ADDSUBC, "ucat_add", "coadmin.php");                  
            ...
            $cname_text = new RcxFormText("", "cname", 25, 25, $cname);
                ...
                $ucat_select->addOptionArray(gall_function("makeCnameSelectArray", array ("galli_category", "cname", "cname", "cid", "scid", $op_coad)));
