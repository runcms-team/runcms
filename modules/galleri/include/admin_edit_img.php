...
// ALT tag            
            $new_alt  = $gallts->makeTboxData4Save($_POST['alt']);
            ...
            if ( $new_image != $img_olddat->img() ){
              ...
              if(!rename( "../thumbnails/".$img_olddat->img(), "../thumbnails/".$new_image )){$eh->show("0111");}
                ...
                $store_img->setVar("coment", $new_coment);
// ALT tag
            ...
            $result = $store_img->store();
                if (!$result){
                    ...
                    redirect_header(GAL_ADMIN_URL."/index.php?op=img_conf&cid=".$new_cid, 1, _AD_MA_DBUPDATED);
                }
          break;
        ...
        echo "<table width=98% border=0 cellspacing=0 cellpadding=0>";
        echo "<tr><td>";
        $form = new RcxThemeForm(_AD_MA_EDITIMG, "editImgS", "index.php");
        ...
        if ( $galerieConfig['safe_mode'] == 0 ){
          if ($copy == 0){
            $copy_button = new RcxFormLabel("", "<img src='".IMG_URL."/pixel.gif' width='20' height='20'><a href='".GAL_ADMIN_URL."/index.php?op=solo_img_copyr&img_id=".$img_Dat->id()."&ret=1'><img src='".IMG_URL."/copyr.jpg' alt='"._AD_MA_COPYRGEN."' width='20' height='20' border='0'></a>");
...
// ALT tag
        $alt_text = new RcxFormText(_AD_MA_ALT, "alt", 80, 254, $img_Dat->alt());
        ...
        $retour_button = new RcxFormLabel("", "<input type='button' class='button' value='"._AD_MA_GOBACK."' onClick=\"self.location.href='index.php?op=img_conf&cid=".$img_Dat->cid()."'\">");
