...
if ( !eregi("gall_page", $galerieConfig['page_type']) ) {
  ...
  exit();
    ...
    $op_hidden = new RcxFormHidden("op","solo_img_copyr");
  ...
  $form->addElement($imgid_hidden);
    ...
    if ($galerieConfig['test_logotyp'] == 1){
        ...
        if ( count($logo_list) > 0){
            $logo_select = new RcxFormSelect(_AD_WZ_LOGO, "test_logo", $galerieConfig['test_logo']);
  ...
  $wz_li_re_select = new RcxFormSelect(_AD_WZ_HELPTEXT2." 1", "test_wz_li_re", $galerieConfig['test_wz_li_re']);
  $wz_li_re_select->addOptionArray(array("0"=>_AD_WZ_LINKS, "1"=>_AD_WZ_CENTER, "2"=>_AD_WZ_RECHTS));
  $form->addElement($wz_li_re_select);
  $wz_ob_un_select = new RcxFormSelect(_AD_WZ_HELPTEXT2." 2", "test_wz_ob_un", $galerieConfig['test_wz_ob_un']);
  $wz_ob_un_select->addOptionArray(array("0"=>_AD_WZ_OBEN, "1"=>_AD_WZ_CENTER, "2"=>_AD_WZ_UNTEN));
  $form->addElement($wz_ob_un_select);
  if ($galerieConfig['test_logotyp'] == 0){
        $wz_richtung_select = new RcxFormSelect(_AD_WZ_ANZ, "test_wz_richtung", $galerieConfig['test_wz_richtung']);
      $wz_richtung_select->addOptionArray(array("0"=>_AD_WZ_HOR, "1"=>_AD_WZ_VERT));
      $form->addElement($wz_richtung_select);
        $wz_text_text = new RcxFormText(_AD_WZ_TEXT, "test_wz_text", 30, 40, $galerieConfig['test_wz_text']);
        $form->addElement($wz_text_text);
      $wz_font_select = new RcxFormSelect(_AD_WZ_FONT, "test_wz_font", $galerieConfig['test_wz_font']);
      ...
      $wz_font_r_text = new RcxFormText(_AD_WZ_FONTR, "test_wz_font_r", 4, 3, $galerieConfig['test_wz_font_r']);
        $form->addElement($wz_font_r_text);
      $wz_font_g_text = new RcxFormText(_AD_WZ_FONTG, "test_wz_font_g", 4, 3, $galerieConfig['test_wz_font_g']);
        $form->addElement($wz_font_g_text);
      $wz_font_b_text = new RcxFormText(_AD_WZ_FONTB, "test_wz_font_b", 4, 3, $galerieConfig['test_wz_font_b']);
        $form->addElement($wz_font_b_text);
  }
    $op_hidden = new RcxFormHidden("op","solo_img_copyr");
  $imgid_hidden = new RcxFormHidden("img_id",$img_id);
    $op_pref_hidden = new RcxFormHidden("op_pref","save");
    $submit_button = new RcxFormButton("", "button", _AD_MA_PREVSTORE, "submit");
    $form->addElement($op_hidden);
  $form->addElement($imgid_hidden);
    $form->addElement($op_pref_hidden);
    $form->addElement($submit_button);  
  $form->setRequired(array("test_wz_text", "test_wz_font_r", "test_wz_font_g", "test_wz_font_b"));
    $form->display();
?>