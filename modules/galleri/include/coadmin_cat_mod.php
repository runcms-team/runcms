                    ...
                    if ($_FILES[$userfile]['name'] != ""){
                      ...
                      $result = $upload->upload();     
                        if ($result[$userfile]['filename']) {
                            ...
                            gall_cp_header(); 
                            gall_function("admin_meldung_go_hidden", array(_ERROR_FONT._Errorup.": ".$result[$userfile]['basename']."<br>".$upload->errors()."</font>", GAL_ADMIN_URL."/coadmin.php", array("name='op' value='cat_conf'", "name='op_coad' value='".$op_coad."'"), "", _AD_MA_NEXT));
                          gall_cp_footer();
                            exit();
                        }
                    }                   
                    redirect_header("coadmin.php?op=cat_conf&op_coad=".$op_coad." ", 1, _AD_MA_DBUPDATED);
                }
          break;
...
<?php        
        ...
        gall_cp_header();
          ...
          $cid =  $_GET['cid'];
            ...
            include_once(GALLI_PATH."/class/module.textsanitizer.php");    
          $gallts =& GallTextSanitizer::getInstance(); 
        titel_Verz(_AD_MA_TITEL, _AD_MA_TITEL2, "cat_conf", 6);
        ...
        $button = $gallts->makeTboxData4Edit($cat_Dat->button());
            OpenTable();
            $form = new RcxThemeForm(_AD_MA_MODCAT, "cat_mod", "coadmin.php");
            ...
            $cname_select->addOptionArray(gall_function("makeCnameSelectArray", array ("galli_category", "cname", "cname", "cid", "scid", $op_coad)));
            ...
            if ( count($button_list) > 0){
                $button_logo_tray = new RcxFormElementTray(_AD_MA_PREV, "");
            ...
            if ($cid != $op_coad){
                ...
                $delet_button->setExtra("onClick=\"location='coadmin.php?op=delCat&cid=".$cid."&op_coad=".$op_coad."'\"");
                $button_tray->addElement($delet_button);  
            }
            $retour_button = new RcxFormButton("", "button", _AD_MA_GOBACK, "button");
            $retour_button->setExtra("onClick=\"location='coadmin.php?op=cat_conf&op_coad=".$op_coad."'\"");
